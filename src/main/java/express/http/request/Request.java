...
public class Request {
  ...
  private final HashMap<String, String>
      formQueries; // Form query parameters (application/x-www-form-urlencoded)
  ...
  public Request(HttpExchange exchange, Express express) {
    ...
    String contentLength =
        headers.get("Content-Length") != null ? headers.get("Content-Length").get(0) : null;
    this.contentLength = contentLength != null ? Long.parseLong(contentLength) : -1;

    // Check if the request contains an body-content
    this.contentType =
        headers.get("Content-Type") == null ? "" : headers.get("Content-Type").get(0);
  ...
  /**
   * Pipe the body from this request to an file. If the file not exists, it will be created.
   *
  ...
  /**
   * Get the data from a specific middleware by name (Also the reason why the interface
   * ExpressFilter implements a getName())
   *
