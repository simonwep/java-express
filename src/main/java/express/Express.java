...
public class Express implements Router {
  ...
  /**
   * Create an express instance and bind the server to an hostname. Default is "Localhost"
   *
  ...
  /**
   * Add an key-val pair to the express app, can be used to store data. Uses ConcurrentHashMap so
   * it's thread save.
   *
   * @param key The key
   * @param val The value
  ...
  /**
   * Set an executor service. Default is CachedThreadPool Can only changed if the server isn't
   * already stardet.
   *
  ...
  /**
   * Binds a or multiple objects with request-handler methods on this express instance. With the use
   * of the DynExpress Annotation handler can be declared with a annotation on a compatible method
   * which has as parameter a Request and Response object.
   *
    ...
    for (Object o : objects) {
      ...
      for (Method method : methods) {
        ...
        if (params.length < 1 || params[0] != Request.class || params[1] != Response.class) {
          ...
          System.err.println(
              "Skipped method with invalid parameter types found in "
                  + method.getName()
                  + "("
                  + paramString
                  + ") in "
  ...
  /**
   * Start the HTTP-Server on port 80. This method is asynchronous so be sure to add an listener or
   * keep it in mind!
   */
  public void listen() {
    listen(null, 80);
